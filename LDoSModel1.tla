...
CONSTANT Sender, Receiver, AttackerPacket, Router, Packet
...
VARIABLES pc, now, mss, sequencenumber, window_size, data, router_queue, 
          ...
          attacker_data, attacker_pc, attacker_ubTimer, attacker_lbTimer
...
/\router_queue           = 1000
\*Attacker
/\attacker_pc              = [a \in AttackerPacket |-> "Attacker"]
/\attacker_data            = [a \in AttackerPacket |-> 1000]
/\attacker_ubTimer         = [a \in AttackerPacket |-> Infinity]
/\attacker_lbTimer         = [a \in AttackerPacket |-> 0]
...
/\UNCHANGED<<pc, now, mss, sequencenumber, window_size, data, router_queue, packet_ubTimer,
             ...
             packet_lbTimer, attacker_data, attacker_pc, attacker_ubTimer, attacker_lbTimer>>
...
/\UNCHANGED<<router_queue, sequencenumber, now, mss, received_data, received_sequencenumber,
             ...
             rtpacket_ubTimer, rtpacket_lbTimer, attacker_data, attacker_pc, attacker_ubTimer,
             attacker_lbTimer>>
...
/\UNCHANGED<<received_data, received_sequencenumber, attacker_data, attacker_pc,
             attacker_ubTimer, attacker_lbTimer>>
...
/\UNCHANGED<<attacker_data, attacker_pc, attacker_ubTimer, attacker_lbTimer>>  
      ...
      /\ \/ /\IF (router_queue - rtpacket_data[p] >= 0)
                 ...
                 ELSE
                     ...
                     /\UNCHANGED<<router_queue, sequencenumber, now, data, mss, window_size,
                                  ...
                                  packet_lbTimer, rtpacket_lbTimer>>     
/\UNCHANGED<<received_data, received_sequencenumber, attacker_data, attacker_pc, attacker_ubTimer, 
             attacker_lbTimer>>      
...
/\IF ((now % 1000) = 0)
    THEN
        /\At(a, "Attacker", attacker_pc)
        /\GoFromTo(a, "Attacker", "AttackerSendingToRouter", attacker_pc)
        /\SetTimer(a, attacker_lbTimer, 100)
        /\SetTimer(a, attacker_ubTimer, 100)
        /\UNCHANGED<<pc, now, mss, sequencenumber, window_size, data, router_queue, 
                     packet_ubTimer, packet_sequencenumber, packet_data,received_data,
                     received_sequencenumber, rto, rto_countTimer, rtpacket_sequencenumber,
                     rtpacket_data, rtpacket_pc, rtpacket_ubTimer, packet_lbTimer, 
                     rtpacket_lbTimer, attacker_data>> 
    ELSE
        /\UNCHANGED<<pc, now, mss, sequencenumber, window_size, data, router_queue, 
                     packet_ubTimer, packet_sequencenumber, packet_data, received_data,
                     received_sequencenumber, rto, rto_countTimer, rtpacket_sequencenumber,
                     rtpacket_data, rtpacket_pc, rtpacket_ubTimer, packet_lbTimer, 
                     rtpacket_lbTimer, attacker_data, attacker_pc, attacker_ubTimer, 
                     attacker_lbTimer>>
               
AttackerSendingToRouter(a) ==
/\SetTimer(a, attacker_ubTimer, 100)
/\TimedOut(a, attacker_lbTimer)
/\SetTimer(a, attacker_lbTimer, 100)
/\GoFromTo(a, "AttackerSendingToRouter", "AttackerRouters", attacker_pc)
/\router_queue' = router_queue - attacker_data[a]
/\UNCHANGED<<pc, now, mss, sequencenumber, window_size, data, packet_ubTimer,
             packet_sequencenumber, packet_data, received_data, received_sequencenumber, 
             rto, rto_countTimer, rtpacket_sequencenumber, rtpacket_data, rtpacket_pc,
             rtpacket_ubTimer, packet_lbTimer, rtpacket_lbTimer, attacker_data>>

AttackerRouters(a) ==
/\SetTimer(a, attacker_ubTimer, 100)
/\TimedOut(a, attacker_lbTimer)
/\SetTimer(a, attacker_lbTimer, 100)
/\GoFromTo(a, "AttackerRouters", "AttackerSendingToReceiver", attacker_pc)
/\router_queue' = router_queue + attacker_data[a]
/\UNCHANGED<<pc, now, mss, sequencenumber, window_size, data, packet_ubTimer, 
             packet_sequencenumber, packet_data, received_data, received_sequencenumber,
             rto, rto_countTimer, rtpacket_sequencenumber, rtpacket_data, rtpacket_pc,
...
AttackerSendingToReceiver(a) == 
/\SetTimer(a, attacker_ubTimer, 100)
/\TimedOut(a, attacker_lbTimer)
/\SetTimer(a, attacker_lbTimer, 100)
/\GoFromTo(a, "AttackerSendingToReceiver", "AttackerReceivers", attacker_pc)
/\UNCHANGED<<pc, now, mss, sequencenumber, router_queue, window_size, data, packet_ubTimer, 
             packet_sequencenumber, packet_data, received_data, received_sequencenumber,
             rto, rto_countTimer, rtpacket_sequencenumber, rtpacket_data, rtpacket_pc,
             rtpacket_ubTimer, packet_lbTimer, rtpacket_lbTimer, attacker_data>>
...
AttackerReceivers(a) == 
/\TimedOut(a, attacker_lbTimer)
/\ResetUBTimer(a,attacker_ubTimer)
/\GoFromTo(a, "AttackerReceivers", "Attacker", attacker_pc)
/\UNCHANGED<<pc, now, mss, sequencenumber, router_queue, window_size, data, packet_ubTimer,
             packet_sequencenumber, packet_data, received_data, received_sequencenumber,
             rto, rto_countTimer, rtpacket_sequencenumber, rtpacket_data, rtpacket_pc, 
             rtpacket_ubTimer, packet_lbTimer, rtpacket_lbTimer,
             attacker_data, attacker_lbTimer>>
...
\E d \in PosReal:
      ...
      /\ \A a \in AttackerPacket: attacker_ubTimer[a] >= d
      ...
      /\ attacker_ubTimer' = [a \in AttackerPacket |->
                          IF attacker_ubTimer[a] = Infinity THEN Infinity
                                                            ELSE attacker_ubTimer[a] - d]
      
      /\packet_lbTimer'      = [p \in Packet |-> Max(0, packet_lbTimer[p] - d)]
      /\rtpacket_lbTimer'    = [rp \in Packet |-> Max(0, rtpacket_lbTimer[rp] - d)]
      /\attacker_lbTimer'    = [a \in AttackerPacket |-> Max(0, attacker_lbTimer[a] - d)]
      ...
      /\UNCHANGED <<pc, sequencenumber, data, window_size, mss, router_queue, packet_data, 
                    packet_sequencenumber, received_data, received_sequencenumber, rto, 
                    rtpacket_sequencenumber, rtpacket_data, rtpacket_pc, attacker_data, attacker_pc>>


Terminating == /\ \A p \in Packet: pc[p] ="End"
               /\ \A p \in Packet: rtpacket_pc[p] ="NotExist"
               /\ \A a \in AttackerPacket: attacker_pc[a] = "NotExist"
               /\UNCHANGED vars 

UDP(a) == \/Attackers(a)\/AttackerSendingToRouter(a)\/AttackerRouters(a)
          \/AttackerSendingToReceiver(a)\/AttackerReceivers(a)

SPNext(s,p,r,a) == \/Senders(s,p)\/SendingToRouter(p)\/Routers(p)\/SendingToReceiver(p)
                   \/Receivers(s,p,r)\/SendingToRouterRe(p)
                   \/RoutersRe(p)\/SendingToSenderRe(s,p)
                   \/ReTransmissionTimeout(s,p)\/UDP(a)
                   
Next ==  Tick \/ (\E s \in Sender: 
                      \E p \in Packet: 
                           \E r \in Receiver: 
                                \E a \in AttackerPacket:
                                                SPNext(s, p, r, a) )
...
Liveness ==
    /\ \A s \in Sender: 
             \A p \in Packet: 
                  \A r \in Receiver:
                            \A a \in AttackerPacket:
                                    WF_vars( SPNext(s, p, r, a) )
    /\SF_vars(Tick)
...
Inv6 ==
    /\ \A a \in AttackerPacket: attacker_pc[a] # "AttackerReceivers"

=============================================================================
\* Modification History
\* Last modified Mon Jan 15 17:01:04 JST 2024 by yamatani
\* Created Mon Jan 15 13:11:28 JST 2024 by yamatani
